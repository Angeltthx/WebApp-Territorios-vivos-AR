...
- **`view`, `facing` and `iconSize` in the catalog are per-animal on purpose, and all three were set by looking, not by reasoning.** `view` picks which face you see (`'side'` for the whale and guan, drawn in profile and unrecognisable from above; `'top'` for the turtle, drawn in plan; the crab uses `'side'` + `facing: 90` so it faces the camera). `facing` is degrees of yaw about the icon's own up-axis, and it exists because each model was sculpted pointing wherever its author felt like — the whale needed 180° to face the same way as its drawing, the turtle −90°. `iconSize` multiplies `ICON_TARGET_SIZE` in `IconLoader`. `MarkerPin.applyView` + `MarkerPin.sync` are the only places these become rotations.
  **Calibrate them in `/verify.html`, never analytically.** The on-screen size of an icon is *not* `ICON_TARGET_SIZE × iconSize`: `fitToIconSize` normalises the model's largest 3D dimension, and whether that dimension ends up across the screen or pointing at the camera depends on `view` and `facing`. The guan needed `iconSize: 2.7` to match a drawing that is 0.23 map-widths tall. Measure against the drawing on the page; do not compute it.
